...
final class ClockStrikeTests: SkipUnavailableTestCase {
    ...
    func testPredicateStrike() {
        ...
        let fastClock = Clocks.custom(
            startingFrom: justBeforeNextMinute.firstInstant,
            rate: 60.0,
            region: clock.region
        )
    ...
    func testPredicateStrikeAsync() async throws {
        ...
        let fastClock = Clocks.custom(
            startingFrom: justBeforeTheNextMinute.firstInstant,
            rate: 60.0,
            region: clock.region
        )
        var results = [0, 13, 26, 39, 52]

        for try await time in fastClock.strike(
            producing: Second.self,
            when: { $0.second.isMultiple(of: 13) }
        ).asyncValues {
...
final class ClockStrikeTests: XCTestCase {
    static var allTests: [(String, (ClockStrikeTests) -> () throws -> Void)] = []
}
