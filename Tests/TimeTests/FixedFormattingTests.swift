...
class FixedFormattingTests: SkipUnavailableTestCase {
    ...
    override func setUpWithError() throws {
        ...
        clock = Clocks.custom(
            startingFrom: Instant(interval: 0, since: .reference),
            rate: 0.001,
            region: .posix
        )
    }
    ...
    func testMonthFormatting() {
        ...
        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .naturalName),
            "January 2001 AD"
        )
        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .abbreviatedName),
            "Jan 2001 AD"
        )
        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .narrowName),
            "J 2001 AD"
        )
        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .naturalDigits),
            "1 2001 AD"
        )
        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .twoDigits),
            "01 2001 AD"
        )
    }
    ...
    func testDayFormatting() {
        ...
        XCTAssertEqual(
            v.format(year: .naturalDigits, month: .naturalName, day: .naturalDigits),
            "January 1, 2001"
        )
        XCTAssertEqual(
            v.format(year: .naturalDigits, month: .naturalName, day: .twoDigits),
            "January 01, 2001"
        )

        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .naturalName, day: .naturalDigits),
            "January 1, 2001 AD"
        )
        XCTAssertEqual(
            v.format(era: .abbreviated, year: .naturalDigits, month: .naturalName, day: .twoDigits),
            "January 01, 2001 AD"
        )
        ...
        XCTAssertEqual(
            v.format(year: .naturalDigits, month: .naturalName, day: .twoDigits, weekday: .naturalName),
    ...
    func testRawFormatting_Lenient() {
        ...
        XCTAssertEqual(
            try v.format(raw: "y-MM-dd 'at' HH:mm:ss", strict: false),
            "2001-01-01 at 00:00:00"
        )
    }

    func testOmittedTimeZoneInDescription() throws {
        let fixedNano = try Fixed(
            region: .posix,
            year: 2024,
            month: 3,
            day: 3,
