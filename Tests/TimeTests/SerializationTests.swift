...
class SerializationTests: SkipUnavailableTestCase {
    ...
    func testCodableTimePeriodRoundTrip() throws {
        ...
        func testRoundTrip<U: Unit>(
            of timePeriod: Fixed<U>,
    ...
    func testMaliciousPayload() throws {
        ...
        let validPayload = generatePayloadFor(
            year: 2023,
            month: 01,
            day: 01,
            hour: 11,
        ...
        let invalidMonthPayload = generatePayloadFor(
            year: 2023,
            month: -4,
            day: 01,
            hour: 11,
        ...
        XCTAssertThrowsError(
            try decoder.decode(Fixed<Second>.self, from: invalidMonthPayload),
            "-4 is an invalid calendar month"
        )
        ...
        XCTAssertThrowsError(
            try decoder.decode(Fixed<Second>.self, from: invalidHourPayload),
            "27 is an invalid hour"
        )

        let invalidDayPayload = generatePayloadFor(
            year: 2023,
            month: 01,
            day: 00,
            hour: 01,
        ...
        XCTAssertThrowsError(
            try decoder.decode(Fixed<Second>.self, from: invalidDayPayload),
            "0 is an invalid day"
        )

        let payloadMissingPayload = generatePayloadFor(
            year: 2023,
            month: 01,
            day: 01,
            hour: 01,
        ...
        XCTAssertThrowsError(
            try decoder.decode(Fixed<Nanosecond>.self, from: payloadMissingPayload),
            "Nanoseconds are missing"
        )
    }

    private func generatePayloadFor(
        year: Int,
        month: Int,
        day: Int,
        hour: Int,
