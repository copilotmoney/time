...
class LinuxIssues: XCTestCase {
    ...
    func testFormatting() throws {
        ...
        let allFormats: [(Format, String, String, StaticString, UInt)] = [
            ...
            (
                Template<TimeZone>.ISO8601WithHoursAndMinutes(extended: false, includingZ: false), "+0000",
                "+0000", #file, #line
            ),
            (
                Template<TimeZone>.ISO8601WithHoursAndMinutes(extended: false, includingZ: true), "Z", "Z",
                #file, #line
            ),
            (
                Template<TimeZone>.ISO8601WithHoursAndMinutes(extended: true, includingZ: false), "+00:00",
                "+00:00", #file, #line
            ),
            (
                Template<TimeZone>.ISO8601WithHoursAndMinutes(extended: true, includingZ: true), "Z", "Z",
                #file, #line
            ),
        ]
        ...
        for (format, expectedRaw, expectedTemplate, file, line) in allFormats {
            ...
            XCTAssertEqual(
                rawFormatted,
                expectedRaw,
                "Raw format '\(format.template)' produced '\(rawFormatted)' instead of '\(expectedRaw)'",
                file: file,
                line: line
            )
            XCTAssertEqual(
                templateFormatted,
                expectedTemplate,
                "Template '\(format.template)' produced '\(templateFormatted)' instead of '\(expectedTemplate)'",
                file: file,
                line: line
            )
        }
