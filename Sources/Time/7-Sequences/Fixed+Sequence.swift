...
public struct FixedSequence<U: Unit & LTOEEra>: Sequence {
    ...
    public init(
        start: Fixed<U>,
        stride: TimeDifference<U, Era>,
        while keepGoing: @escaping (Fixed<U>) -> Bool
    ) {
    ...
    public init<S>(range: Range<Fixed<S>>, stride: TimeDifference<U, Era>) {
        ...
        let upper =
            range.upperBound.region.isEquivalent(to: lower.region)
            ? range.upperBound : Fixed<S>(region: lower.region, instant: range.upperBound.firstInstant)
        constructor = {
            FixedIterator(
                region: lower.region,
                range: lower.firstInstant..<upper.firstInstant,
                stride: stride
            )
        }
    }
    ...
    internal init<S>(
        parent: Fixed<S>,
        stride: TimeDifference<U, Era> = TimeDifference(value: 1, unit: U.component)
    ) {
...
public struct FixedIterator<U: Unit & LTOEEra>: IteratorProtocol {
    ...
    public init(
        start: Fixed<U>,
        stride: TimeDifference<U, Era>,
        keepGoing: @escaping (Fixed<U>) -> Bool
    ) {
