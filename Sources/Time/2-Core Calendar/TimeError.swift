...
extension TimeError {
    ...
    internal static func missingCalendarComponents(
        _ units: Set<Calendar.Component>,
    ...
    internal static func invalidDateComponents(
        ...
        return .init(
            ...
            description: description
                ?? "The provided date components (\(components)) cannot be correctly interpreted in the \(region) region"
        )
    }

    internal static func invalidFormatString(
        _ string: String,
        units: Set<Calendar.Component>? = nil,
        description: String? = nil
    ) -> TimeError {
    ...
    internal static func cannotParseString(
        ...
        if let description {
            ...
            resolvedDescription =
                "Cannot parse string '\(valueString)' with format '\(formatString)' in the \(region) region"
        } else {
            resolvedDescription = "Cannot parse string '\(valueString)' in the \(region) region"
        }

        return .init(
            reason: .invalidValueString,
            units: units,
            dateComponents: nil,
            region: region,
