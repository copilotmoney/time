...
extension Fixed {
    ...
    internal func value(for unit: Calendar.Component) -> Int {
        return dateComponents.value(for: unit).unwrap(
            "A Fixed<\(Granularity.self)> does not contain a represented \(unit)"
        )
    }
    ...
    internal func last<U: Unit>() -> Fixed<U> {
        return Fixed<U>(region: region, instant: range.upperBound)
            - TimeDifference(value: 1, unit: U.component)
    }
    ...
    internal func nth<U: Unit>(_ ordinal: Int) throws -> Fixed<U> {
        ...
        guard ordinal >= 1 else {
            throw TimeError.invalidDateComponents(
                target,
                in: region,
                description: "Invalid ordinal of \(ordinal) \(U.component)"
            )
        }
        ...
        guard parentRange.lowerBound <= childRange.lowerBound else {
            throw TimeError.invalidDateComponents(
                target,
                in: region,
                description: "\(U.component) #\(ordinal) is outside the range of \(self)"
            )
        }
    ...
    internal func computeDifference<Min: Unit, Max: Unit>(to other: Fixed<Granularity>)
    ...
    internal func round<U: LTOEEra>(to unit: U.Type = U.self, direction: RoundingDirection) -> Self {
        ...
        let nearestStart =
            (intervalToLowerDown <= intervalToUpStart) ? roundedDownStart : roundedUpStart
        return Self(region: self.region, instant: nearestStart)
    }

    internal func roundToMultiple(
        of match: TimeDifference<Granularity, Era>,
        direction: RoundingDirection
    ) -> Self where Granularity: LTOEYear {
        /*
         PREMISE:
        ...
        guard
            let smallest = Calendar.Component.ascendingOrder.first(where: { represented.contains($0) })
        else {
            // throw?
            return self
        }
    ...
    internal func format(_ templates: [Format?]) -> String {
