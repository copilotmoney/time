...
extension Fixed {
    ...
    public func applying(difference: TimeDifference<Granularity, Era>) -> Self {
        ...
        let newDate = self.calendar.date(byAdding: diff, to: d).unwrap(
            "Unable to add \(diff) to \(self)"
        )
...
extension Fixed where Granularity: LTOENanosecond {
    ...
    public func adding(nanoseconds: Int) -> Self {
        return applying(difference: .nanoseconds(nanoseconds))
    }
    ...
    public func subtracting(nanoseconds: Int) -> Self {
        return applying(difference: .nanoseconds(-nanoseconds))
    }
