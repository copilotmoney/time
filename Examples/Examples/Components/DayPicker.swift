...
struct DayPicker<Label: View>: View {
    ...
    init(
        selection: Binding<Fixed<Day>>,
        consistentNumberOfWeeks: Bool = true,
        @ViewBuilder label: @escaping (Binding<Fixed<Month>>) -> Label
    ) {
    ...
    init(selection: Binding<Fixed<Day>?>, consistentNumberOfWeeks: Bool = true)
    where Label == DefaultFixedDayTitle {
        self._selection = selection
        self.consistentNumberOfWeeks = consistentNumberOfWeeks
        self.label = { DefaultFixedDayTitle(month: $0) }
    }

    private var weeksForCurrentMonth: [[Fixed<Day>]] {
